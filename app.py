...
with tab4:
    ...
    with col_g: 
        st.link_button("📧 إرسال الطلب عبر الايميل (Email)", f"https://mail.google.com/mail/?view=cm&fs=1&to=octanova.team@example.com&su=SheroWhey_Order&body={urllib.parse.quote(msg_template)}", use_container_width=True, disabled=(not st.session_state.want_m and not st.session_state.want_s))
